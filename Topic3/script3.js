...
function renderDevices() {
  $.get(`${API_URL}/devices3`)
    .then(response => {
      $('#devices3 tbody').empty();
      response.forEach(device => {
        $('#devices3 tbody').append(`
          <tr>
            <td>${device.user}</td>
            <td>${device.name}</td>
...
$('#add-device').on('click', () => {
  ...
  $.post(`${API_URL}/devices3`, body)
    .then(response => {
      console.log('eqqqqqqqq');
      window.location.replace('http://localhost:3000/madhav3') ;
    })
    .catch(error => {
...
async function deleteDevice(idToDelete) {
  try {
    ...
    const response = await fetch(`${API_URL}/devices3/${idToDelete}`, {
      method: 'DELETE'
    });
    if (!response.ok) {
...
// Event listener for delete button
$('#devices3 tbody').on('click', '.delete-btn', function () {
